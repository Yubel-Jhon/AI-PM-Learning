...
```
输入层  钉钉输入框 · Alt+Z 热键 · 悬浮窗面板 · 预览窗拍板
  ↓
编排层  抓草稿 → 认人（人物卡 / 记忆库）→ 识别对方招式、定语气档 → 拼提示词
  ↓
引擎层  云端千问（默认）→ 本地 Ollama（隐私档）→ 规则兜底，三级自动降级
  ↓
质检层  格式 lint ＋ 原意保持度（立场/数字/时间）→ 不过带原因重写一次
  ↓
发送闸  预览窗拍板：Enter 发 / Esc 弃 / Alt+R 换一版
```

- **记忆横切**：`context.db`（关系卡 · 近 1 月原文）＋ `persona_cards/` 人物卡——改写自动带前情和分寸，实战发现回写、越用越准
- **自我进化**：经验库沉淀踩坑与盲区（规则级学习回路），复盘改规则，三副本+桌面引擎同步——越用越准的不只是人物卡，还有规则本身
- **skill 三件套**（`skill/`）：同一套规则的手动挡——zuiti-rewrite 改写 · chat-persona 人物分析 · zuiti-evolve 自我进化
- 九步生命线 / 质检闭环 / 降级矩阵 / 接口契约的全景详解见 **[docs/架构图.html](docs/架构图.html)**
