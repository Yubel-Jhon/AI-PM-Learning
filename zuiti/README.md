...
- **可观测性**：每次改写落本地运行日志（只存档位/耗时/违规类别码），`--stats` 一眼看出兜底率、采纳率、手改率
...
每次改写在本地 `script/self-improvement/runs.jsonl` 追加两行记录：一次**尝试**（引擎/耗时/档位/违规类别码/结局：一次过、重写过、小毛病容忍、规则兜底、降级、缓存）+ 一次**拍板**（发出 / 手改后发 / 放弃 / 没抓到草稿）。**只落数字和档位**——原话、成稿、对方是谁、窗口标题一个字不落，文件不入仓库。

- **看汇总**：`python script/zuiti_overlay.py --stats`（默认近 7 天，`--days 30` 换窗口）；`检查环境.bat` 也带这一段
- **追单次**：按 `run` 字段在 runs.jsonl 里筛（一次 Alt+Z = 一个 run，换一版/换档只加 seq）
- **怎么解读**：兜底率 >10% 查引擎 · 平均 >5s 查思考模式坑 · 采纳率 <70% 该场景示例优先复盘 · 手改率高 = 规则盲区（喂 `zuiti-evolve` 复盘）
